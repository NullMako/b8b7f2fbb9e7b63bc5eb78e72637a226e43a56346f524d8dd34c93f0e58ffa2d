...
Хотілось би спробувати проаналізувати цей файл за допомогою VirusTotal:  
...
Якщо ми прогротаємо до самого кінця, ми побачимо `$base64reconstruction`, `$xorrotational`, `$masterdecoder` та `$executionhandler`. Якщо заглибитись у код, стають зрозумілими призначення цих функцій:
...
Потрібно розшифрувати шкідливий код. Але навіщо це робити руками, якщо сам скрипт це може зробити замість нас? Перепишемо функцію `$executionhandler` - замість команд виконнання вставляємо цю команду:  
`$decodedScript | Out-File "C:\Users\Public\decoded_script.ps1"`  
...
## Отримання пейлоаду
І вуаля. Тут ми бачимо нескінченний цикл, що перевіряє наявність процесу `Aspnet_compiler.exe`. Але це нас не так цікавить, як змінні `$assemblyData` та `$ExecutionPayload`.  

Почнемо з `$assemblyData`. Видно що цей рядок закодований Base64. За допомогою утиліти CyberChef декодуємо. Перші два байти (MZ або 77 90) вказують на те, що це виконуваний файл Windows. Зберігаємо файл у вигляді тектового документа(ми ще не знаємо справжнього розширення).  
...
VirusTotal вказує на те, що це стілер з сигнатурою Formbook. Більшого наразі сказати не можу. Можливо, якщо вдасться отримати дамп працюючого процесу, оновлю цей репозиторій.
